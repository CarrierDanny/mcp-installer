...
const AIClient = {
  async chat(messages, options = {}) {
    ...
    const model = options.model || config.ai_models?.[provider];
    const maxTokens = options.max_tokens || 2000;

    switch (provider) {
      case 'claude': return this._claude(messages, apiKey, model, maxTokens);
  ...
  async _claude(messages, apiKey, model, maxTokens) {
    ...
    if (!resp.ok) {
      throw new Error(data?.error?.message || summarizeNonJsonResponse('Claude', contentType, raw) || `Claude API error: ${resp.status}`);
    }
    if (!data) {
