// background/soql-engine.js — CellsForce SOQL builder (v6.9)
(function (global) {
  'use strict';

  const CATALOG_URL = 'data/soql-catalog.json';
  const STORAGE_TEMPLATES = 'gpd_soql_hot_slots';
  const MAX_CASES = 200;
  ...
  function getCatalogUrl() {
    try {
      return chrome.runtime.getURL(CATALOG_URL);
    } catch (_) {
      return CATALOG_URL;
    }
  }
  ...
  async function getHotSlots() {
    return new Promise((resolve) => {
      chrome.storage.local.get([STORAGE_TEMPLATES], (r) => {
  ...
  async function saveHotSlot(slotIndex, data) {
    ...
    return new Promise((resolve) => {
      chrome.storage.local.set({ [STORAGE_TEMPLATES]: slots }, () => resolve({ ok: true, slots }));
    });
  }
