...
const Logger = {
  ...
  // Level-style helpers. SetupWizard and memory.js log with
  // logger.info/warn/error(message); map those onto the (action, detail, data)
  // entry shape so they land in real fields instead of crashing.
  async info(message, data = null) { return this.log('INFO', message, data); },
  async warn(message, data = null) { return this.log('WARN', message, data); },
  async error(message, data = null) { return this.log('ERROR', message, data); },
  async debug(message, data = null) { return this.log('DEBUG', message, data); },
